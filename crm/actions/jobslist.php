<?php
		...
		if ($searchInServiceArea > 0 && $form->elementExists('search_in_service_area')) {
		...
		{	
			...
			if ( preg_match("/\b\d{4}\b/",$postcode) ) {// check for a four digit number at least 
				$sql = "SELECT * FROM postcodeProximity WHERE (postcode=$postcode or postcode2=$postcode) AND proximity<=$distance";
	//echo dumper($sql);
				$postcodeObj = &new postcode();
				$postcodeDAO = $postcodeObj->getRecordsFromQuery($sql);
				while($postcodeDAO->fetch())
				{
					if ($postcodeDAO->postcode == $postcode) {
						$postcodeArray[] =$postcodeDAO->postcode2;
					} elseif ($postcodeDAO->postcode2 == $postcode) {
						$postcodeArray[] = $postcodeDAO->postcode;
					}
				}
			}
