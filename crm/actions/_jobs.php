...
switch ($action)
{
	...
	case "jobadd":
		...
		// :: elements
		$form = &new HTML_QuickForm('jobadd','POST','index.php?_a=jobadd');
...
//		$form->addElement('textarea', 'fullDescription',null, array('cols'=>30,'rows'=>6));
	...
	case "jobedit":
		...
		// :: elements
		$form = &new HTML_QuickForm('jobedit','POST','index.php?_a=jobedit&listingID='.$listingID);
		$form->addElement('hidden', 'status','temp');
		$form->addElement('hidden', 'listingType','job');
		$form->addElement('text','title',null, array('size'=>40));
		$form->addElement('textarea', 'shortDescription',null, array('cols'=>30,'rows'=>3));
//		$form->addElement('textarea', 'fullDescription',null, array('cols'=>30,'rows'=>6));
	...
	case "jobconfirm":
		...
		#++++ FIND USER IS TRUSTED OR NOT
		$accountsDAO = &new account();
		$accountArray = $accountsDAO->getOneRecordArray($accountID);

		if(($accountArray['trustLevel']=="jobs") || ($accountArray['trustLevel']=="all")){
			$listingupdArray=$otherValues = array('status'=>'approved');
			$listingupdDAO = &new listing();
			$listingupdDAO->updateRecord($listingID, $listingupdArray);
		}


		#++++ SPONSORED BUSINESS LISTING
//		$sponsBusDAO = getBusinessesFromListing($listingID,$listingDAO->responseLimit); // [listingID, no of records to get (record limit) ]
//		$sponsBusDAO = getCategoriesFromListing($listingID,$listingDAO->responseLimit);
				$sponsBusDAO = make_array_rank($listingID);
//		$sponscatDAO->fetch();

		//$sponsBusDAO->fetch();
//		while($sponsBusDAO->fetch())
//			{
//				$accountID=$sponsBusDAO->accountID;
//				This function is used for job alert related to businees
//					check_data($accountID,$sponsBusDAO->businessID);
//			}
	...
	case "jobconfirmed":
		...
		$cond_alert = " WHERE accountID='$accountID' AND listingID='$listingID'";

		#++++ SQL QUERY FOR LIST
		$sql_alert = "SELECT DISTINCT * FROM listings $cond_alert";
		//print "<br>$sql_alert<br>";
		$listingObj = &new listing();
		$listingDAO = $listingObj->getRecordsFromQuery($sql_alert);
		$listingDAO->fetch();

//		$sponsBusDAO = getBusinessesFromListing($listingID,$listingDAO->responseLimit); // [listingID, no of records to get (record limit) ]
/*		$sponscatDAO = getCategoriesFromListing($listingID,$listingDAO->responseLimit);
		$sponscatDAO->fetch();
		while($sponsBusDAO->fetch())
			{
				if($status=='approved'){			
...
//					check_data($accountID,$sponsBusDAO->businessID);
		...
		$cond = $cond." WHERE listingID='$listingID' AND listingType='job'";
		
		if($accountDAO->trustLevel=='jobs' || $accountDAO->trustLevel=='all')
			$sql = "UPDATE listings SET status='$status', approveDate=$time $cond";
		else
			$sql = "UPDATE listings SET status='$status' $cond";
		//print "<br>$sql<br>";
		$listingDAO = $listingObj->getRecordsFromQuery($sql);
*/	
